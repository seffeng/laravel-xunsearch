...
return [
    ...
    'databases' => [
        ...
        'default' => [
             ...
             * 默认字符集
             */
            'project.default_charset' => env('XUNSEARCH_SERVER_CHARSET', 'utf-8'),

            /**
             * 索引服务端配置
             */
            'server.index' => env('XUNSEARCH_SERVER_INDEX', 'localhost:8383'),

            /**
             * 搜索服务端配置
             */
            'server.search' => env('XUNSEARCH_SERVER_SEARCH', 'localhost:8384'),
            ...
            'name' => [
                 ...
                 * 也可以指定自定义分词器，格式为 name 或 name(arg) 两种形式，
                 * 其中 name 是分词器名称，arg 则是传递给分词器构造函数的参数。
                 ...
                 * 是否支持精确检索
                 *
                 * 即当给搜索语句加上引号时，则要求匹配的结果必须严格按照搜索词的顺序匹配，
                 * 此外还支持用 NEAR 之类的语法来做精确检索，具体参见：搜索技巧 。
